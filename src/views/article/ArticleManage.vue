<template>
    <el-card class="page-container">
        ...
        <el-table :data="articles" style="width: 100%">
            ...
            <el-table-column label="操作" width="200">
                <template #default="{ row }">
                    ...
                    <el-button :icon="Edit" circle plain type="primary" @click="openUpdateDrawer(row)"></el-button>
        ...
        <!-- 分页条 -->
        <el-pagination v-model:current-page="pageNum" v-model:page-size="pageSize" :page-sizes="[10, 15, 20, 30, 50]"
    ...
    <el-drawer v-model="updateDrawer" title="修改文章" direction="rtl" size="50%">
        <!-- 添加文章表单 -->
        <el-form :model="updateModel" label-width="100px">
            ...
            <el-form-item label="文章内容">
                <div class="editor">
                    <QuillEditor theme="snow" contentType="html" v-model:content="updateModel.content"></QuillEditor>
                </div>
            </el-form-item>
            <el-form-item>
...
import {
    articleAddService, articleEditService,
...
const articles = ref([
    {
        ...
        "state": "草稿",
        "categoryId": 2,
        "createTime": "2024-01-01 11:55:30",
        "updateTime": "2024-01-01 11:55:30"
    }
])
...
const articleList = async () => {
    ...
    for (let i = 0; i < articles.value.length; i++) {
        ...
        article.createTime = article.createTime.replace(/T/g, '   ')
...
let openBlog = async (article) => {
    display.value = article;
    let res = await articleCategoryDetailService(display.value.categoryId);
    display.value.category = res.data.categoryName;
...
let openUpdateDrawer = async (row, state) => {
    updateDrawer.value = true;
    updateModel.value = row;
}
let updateArticle = async (clickstate) => {
    updateModel.value.state = clickstate
    updateModel.value.createTime = updateModel.value.createTime.replace(/\s{3}/g, 'T');
    let result = await articleEditService(updateModel.value)
